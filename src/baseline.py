...
# TF-IDF + Keras baseline training.
# Key changes from previous version:
#  - Split dataset rows BEFORE synthesizing utterances to avoid leakage.
#  - Synthesize utterances only for training rows (test set uses original messages).
#  - Smaller network + L2 regularization + stronger dropout to reduce overfitting.
#  - Save model in native Keras format (.keras) and HDF5 for compatibility.
# """
...
#     """
#     Prepare texts/labels by synthesizing utterances from symptom-list rows.
#     Use this for TRAINING only (to avoid leakage).
#     """
#     texts: List[str] = []
#     labels: List[str] = []
#     for _, row in df.iterrows():
...
#     """
#     Prepare texts/labels from originals only (no synthesis). Use this for TESTING.
#     """
#     texts: List[str] = []
#     labels: List[str] = []
#     for _, row in df.iterrows():
...
#     # --- split rows BEFORE any synthesis to avoid leakage ---
#     stratify_col = df[intent_col] if len(np.unique(df[intent_col])) > 1 else None
#     train_df, test_df = train_test_split(df, test_size=test_size, random_state=random_state, stratify=stratify_col)
#     print(f"Row-split -> train rows: {train_df.shape[0]}, test rows: {test_df.shape[0]}")

#     # Synthesize utterances only for training
#     texts_train, labels_train = _synthesize_for_df(train_df, intent_col, message_col, synth_per_row=synth_per_row)
#     # For test, use natural/original messages only (no synth) to get realistic eval
...
#         # fallback: synth for test rows (keeps separation still)
#         texts_test, labels_test = _synthesize_for_df(test_df, intent_col, message_col, synth_per_row=1)

#     # combine to build TF-IDF on training corpus only (standard practice)
...
#     # map test labels with same encoder (if unseen labels in test, map to a placeholder)
#     try:
#         y_test = le.transform(labels_test)
#     except Exception:
#         # handle unseen labels (should not normally happen if split stratified)
#         y_test = []
#         for lbl in labels_test:
#             if lbl in list(le.classes_):
#                 y_test.append(int(np.where(le.classes_ == lbl)[0][0]))
#             else:
#                 # unknown label -> mark as -1
#                 y_test.append(-1)
...
#     # convert sparse -> dense for Keras (note: may use memory)
...
#     # build and train Keras model (smaller + regularized)
#     model = build_keras_model(input_dim=X_train_arr.shape[1], num_classes=num_classes, hidden_units=128, dropout=0.5)
#     model.summary()

#     ensure_dir(save_dir)
#     # prefer native keras format .keras, but also save HDF5 for backward compatibility
...
#     chk_native = callbacks.ModelCheckpoint(keras_native_path, monitor="val_loss", save_best_only=True, save_weights_only=False, verbose=1)
#     chk_h5 = callbacks.ModelCheckpoint(keras_h5_path, monitor="val_loss", save_best_only=True, save_weights_only=False, verbose=0)
...
#     # filter out any -1 test labels (unseen mapping fallback) from metrics
#     valid_idx = [i for i, lbl in enumerate(y_test) if lbl >= 0]
#     if not valid_idx:
#         print("No valid labeled test samples to evaluate.")
#         valid_y_test = np.array([])
#     else:
#         valid_y_test = y_test[valid_idx]
#     valid_y_pred = np.array(y_pred)[valid_idx] if len(valid_idx) else np.array([])

#     if len(valid_y_test):
#         acc = accuracy_score(valid_y_test, valid_y_pred)
#         inv_labels = le.inverse_transform(sorted(np.unique(valid_y_test)))
#         report = classification_report(valid_y_test, valid_y_pred, target_names=inv_labels, zero_division=0)
#         print("=== Keras baseline training complete ===")
#         print(f"Train samples: {X_train_arr.shape[0]}, Test samples: {X_test_arr.shape[0]}")
#         print(f"Test accuracy: {acc:.4f}")
#         print("Classification report (test):")
#         print(report)
#     else:
#         print("Skipping detailed evaluation because test labels were not valid.")
...
####################################################### he nantr kart aaho aapn vrcha barobr ahe 
# """
# baseline.py

# TF-IDF + Keras baseline training with group-aware splitting.

# Key points:
#  - Group identical normalized messages together and split by group to prevent message-level leakage.
#  - Synthesize utterances only for TRAIN groups.
#  - Use original (natural) messages for TEST evaluation.
#  - Save model in native Keras (.keras) and HDF5 (.h5) for compatibility.
# """
...
#     # --- group-by-message split to avoid message-level leakage ---
#     df = df.copy()
#     df["_group_key"] = df[message_col].apply(_group_key_from_message)

#     # Representative intent (mode) per group - use mode or first if mode missing
...
#         y_test = le.transform(labels_test)
#     except Exception:
#         # fallback if unseen test labels
#         y_test = []
#         for lbl in labels_test:
#             if lbl in list(le.classes_):
#                 y_test.append(int(np.where(le.classes_ == lbl)[0][0]))
...
#     # Evaluate on test (filter out any -1 labels)
#     y_pred_probs = model.predict(X_test_arr, batch_size=batch_size)
#     y_pred = np.argmax(y_pred_probs, axis=1)
#     valid_idx = [i for i, lbl in enumerate(y_test) if lbl >= 0]
#     if valid_idx:
