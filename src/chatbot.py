...
##########################################3 revised in the late night  ####################

# src/chatbot.py (modified)
"""
Revised, robust, importable chatbot helper.
...
Modifications:
- Strip common leading symptom-phrases ("i have", "i have symptoms", "i'm having", "suffering from", ...).
  This avoids echoing the user's raw phrasing and ensures inference is run on the symptom list itself.
- Ensure get_chatbot_response uses the same preprocessed query as handle_query_stateless so metadata/precautions match.
"""
...
# ---------------- Symptom prefix regex (strip leading phrases like "I have", "I have symptoms", "suffering from", etc.)
_SYMPTOM_PREFIX_RE = re.compile(
    r"^(?:i have(?:\s+symptoms)?|i'm having|i am having|i've been having|i have been experiencing|experiencing|suffering from|i have symptoms|symptoms)\b[:\s\-]*",
    re.I,
)
...
# ---------------- Helper: strip common symptom-leading phrases ----------------
def _strip_symptom_prefix(query: str) -> Optional[str]:
    """
    If query begins with common symptom-intro phrases, strip them and return the remainder.
    Otherwise return None.
    Examples matched: "I have ...", "I have symptoms ...", "I'm having ...", "suffering from ...", "symptoms ..."
    """
    if not query:
        return None
    m = _SYMPTOM_PREFIX_RE.match(query.strip())
    if m:
        remainder = query.strip()[m.end():].strip()
...
def handle_query_stateless(engine: "InferenceEngine", query: str, threshold: float = 0.5) -> str:  # type: ignore
    ...
    # 0) If user started with a symptom-intro phrase, strip it and run inference on just the symptom text
    stripped = _strip_symptom_prefix(q)
    if stripped:
        # Run inference directly on the stripped symptom list and return formatted result
        out = engine.infer(stripped)
        if out.get("emergency"):
            return out.get("reply", "")
        if out.get("confidence", 0.0) < threshold:
            clar = out.get("reply", "") or "Can you tell me how long you've had these symptoms?"
    ...
    # 1) Try to extract a topic from the query (handles many phrasings)
    # Only attempt extraction->info path when we have candidate canonical keys (descriptions/precautions)
    topic = extract_topic_from_query(q, candidate_keys=candidate_keys) if candidate_keys else None
    if topic:
        # If topic maps to an existing canonical key, return info / precautions depending on query intent
        # Determine if user explicitly asked for precautions/treatment/symptoms by checking patterns
    ...
    # 2) No direct topic detected — fall back to classifier inference on the raw query
    out = engine.infer(q)
    if out.get("emergency"):
        return out.get("reply", "")
...
def get_chatbot_response(query: str, models_dir: str = "models", threshold: float = 0.5) -> Dict[str, Any]:
    ...
    IMPORTANT: we preprocess "I have ..." style inputs so both the reply and the metadata
    (intent, description, precautions) are derived from the same processed symptom text.
    """
    engine = _get_engine(models_dir=models_dir, threshold=threshold)

    # Preprocess same as handle_query_stateless: if user used symptom-intro phrase, infer on stripped text
    stripped = _strip_symptom_prefix(query)
    if stripped:
        out = engine.infer(stripped)
    else:
        # For non-prefixed queries, infer on the original query (handle_query_stateless may call infer again internally)
        out = engine.infer(query)
    ...
    if isinstance(prec, (list, tuple)):
        ...
        # engine may return dict of disease->prec list; flatten safely
        items = []
